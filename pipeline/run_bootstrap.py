...
def run(
    ...
    Args:
        reference_masks_dir (Path): Directory containing reference cloud masks.
        alg_folders (dict[str, Path]): Dictionary mapping algorithm names to their mask directories.
        random_seed (int): Random seed for reproducibility.
        tile_size (int): Size of tiles for computing MCCs.
        b_scene (int): Number of bootstrap samples at the scene level.
        b_global (int): Number of bootstrap samples at the global level.
    ...
    for alg, path in alg_folders.items():
        # List scenes for this algorithm and intersect with existing set.
        try:
    ...
    message = f"Bootstrap analysis completed successfully. Saved results to {output_dir}."
    logger.info(message)
